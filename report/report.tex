...
Il débute par générer une séquence de bits aléatoire suivant une distribution normale grâce à \mintinline{matlab}{randn()}.
...
Après l'ajout de la séquence de départ, les bits sont codés dans un code BPSK où les 0 deviennent $-1$ et les 1 restent 1.
...
Dans le cas d'un code BPSK, la relation $E_b/N_0$ au SNR s'écrit
...
Nous pouvons dès lors tracer le diagramme BER de notre méthode de transmission en lui adjoignant le BER mathématiquement optimal d'une modulation BPSK défini par l'équation~\ref{eqn:best-ber}. Cette relation est donnée et acceptée.
\begin{equation}
